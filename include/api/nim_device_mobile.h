...
extern"C"
{
	...
	/** @fn void nim_vchat_set_audio_data_sync_cb(nim_vchat_audio_data_sync_cb_func cb, const char *json_extension,const void *user_data);
	* NIM VCHAT DEVICE 音频数据回调,该方法是同步方法
	* @param[in] json_extension 扩展Json string：kNIMDeviceSampleRate
	* @param[in] cb 结果回调见nim_device_def.h
	* @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
	* @return void 无返回值
	*/
	NIM_SDK_DLL_API void nim_vchat_set_audio_data_sync_cb(nim_vchat_audio_data_sync_cb_func cb, const char *json_extension, const void *user_data);
	...
	*/
	NIM_SDK_DLL_API bool nim_vchat_start_audio_mixing(const char* filePath, bool loopback, bool replace, int cycle, float volume);
