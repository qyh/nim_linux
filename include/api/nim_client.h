...
/** @fn 	void nim_client_login_ex(const char *app_key, const char *account, const char *token, nim_login_result_cb_func cb, const void *user_data);
* NIM客户端登录，登录失败时自动执行重试，如果账号密码错误(即error code为302)不进行登录
* @param[in] app_key APP key
* @param[in] account APP account
* @param[in] token 令牌 (在后台绑定的登录token)
* @param[in] cb 登录流程的回调函数， nim_login_result_cb_func回调函数定义见nim_global_def.h
* @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
* @return void 无返回值
*/
NIM_SDK_DLL_API	void nim_client_login_ex(const char *app_key, const char *account, const char *token, nim_login_result_cb_func cb, const void *user_data);
